...
how_many_zeroes <- function(starting_value, rotations_df) {
  
  #Set values
  current_value <- starting_value
  zeroes_tally <- 0
  rotations_df <- rotations_df %>%
    mutate(number = case_when(direction == "L" ~ number * -1, 
                              TRUE ~ number))
  #Need to use modulos for this
  for (i in 1:nrow(rotations_df)) {
    change_value <- rotations_df$number[[i]]
    times_past_zero <- 0
    
    #-----------------Will the dial land right ON zero? If so, add to tally and reset position
    if ((current_value + change_value) %% 100 == 0) {
      current_value <- 0
      zeroes_tally <- zeroes_tally + 1
    }
    #-----------------How many times will the dial go PAST zero 
    if (abs(current_value) + abs(change_value) > 100) {
      #How many times will we go past zero before changing the index?
      times_past_zero <- floor(abs(change_value)/100)
      #Then get the amount to shift the index by
      shift_by <- abs(change_value) %% 100
      #if our shift_by value will bring us past zero, figure out the new index by adding 100
      #if we are going to the right
      if (change_value > 0) {
        #check if we will go past zero
        if (current_value + shift_by > 99) {
          ...
          #if not, then just add the shift by value
        ...
        if (current_value - shift_by < 0) {
          ...
          times_past_zero <- times_past_zero + 1
          current_value <- 101 - shift_past_zero  
        } else {
          #if not, then just add the shift by value
          current_value <- current_value - shift_by
        }
    ...
    zeroes_tally <- zeroes_tally + times_past_zero
  }
  return(zeroes_tally)
}

how_many_zeroes(50, rotations)
...
fresh_or_spoiled <- read_tsv(here("input/day5.txt"), 
                             col_names = "data") 
ranges <- fresh_or_spoiled %>%
  filter(str_detect(data, "-")) %>%
  separate(data, into = c("range_start", "range_end")) %>%
  mutate(across(everything(), as.numeric))

#hmm.... there are 169 rows, and all the numbers in the ranges and ids are divisible by 13 and 169, and even 371293 to make them smaller whole numbers. 

ids <- fresh_or_spoiled %>%
  filter(!str_detect(data, "-")) %>%
  mutate(data = as.numeric(data)) %>%
  pull(data)

how_many_in_range <- function(ids, ranges) {
  num_in_range <- 0
...
```{r}
ranges <- ranges %>%
  arrange(range_start, range_end) %>% 
  mutate(n_in_range = 0,
         overlap_id = NA_character_)

add_overlap_ids <- function(data) {
  id_tally <- 1
for (i in 2:nrow(ranges)) {
  if (ranges$range_start[[i]] <= ranges$range_end[[i-1]]) {
    ranges$overlap_id[[i-1]] <<- id_tally
    ranges$overlap_id[[i]] <<- id_tally
  } else {
    id_tally <- id_tally + 1
  }
}
...
add_overlap_ids(ranges)

ranges_no_overlap <- ranges %>%
  filter(is.na(overlap_id))

ranges_overlap <- ranges %>%
  filter(!is.na(overlap_id)) %>%
  group_by(overlap_id) %>%
  summarize(range_start = min(range_start),
            range_end = max(range_end))

ranges_without_overlap <-
  ranges_no_overlap %>%
  bind_rows(ranges_overlap)

for (i in 1:nrow(ranges_without_overlap)) {
  ranges_without_overlap$n_in_range[[i]] <- length(ranges_without_overlap$range_start[[i]]:ranges_without_overlap$range_end[[i]])
}

sum(ranges_without_overlap$n_in_range) 

#436164823559766 is too high
#423255925124235 is also too high
#353603024494763 is still too high!
#353507173555373 is the right answer!
```
...
homework <- read_tsv(here("input/day6.txt"), col_names = "q") %>%
  mutate(q = str_squish(q)) %>% 
  separate_wider_delim(q, delim = " ", names_sep = "")

questions <- homework %>%
  filter(row_number() < nrow(homework)) %>%
  mutate(across(everything(), as.numeric))

operators <- homework %>%
  filter(row_number() == nrow(homework)) %>%
  pivot_longer(everything(), names_to = "question", values_to = "operator") %>%
  pull(operator)


do_homework <- function(questions, operators) {
  answers <- c()
  for (i in 1:ncol(questions)) {
    question <- pull(questions[, i])
    operator <- operators[[i]]
    
    if (operator == "+") {
      answers <- c(answers, (sum(question)))
    } else if (operator == "*") {
      answers <- c(answers, (prod(question)))
    }
  }
  return(sum(answers))
}

do_homework(questions, operators)
...
homework <- read_lines(here("input/day6.txt"))
operators_raw <- homework[[length(homework)]]
operators <- str_extract_all(homework[[length(homework)]], "\\*|\\+")[[1]]
operator_positions <- c(str_locate_all(operators_raw, "[\\*|\\+]")[[1]][,1], max(str_length(homework))+1)
